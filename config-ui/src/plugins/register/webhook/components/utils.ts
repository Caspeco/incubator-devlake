...
export const transformURI = (prefix: string, webhook: IWebhook, apiKey: string) => {
  return {
    ...
    postPullRequestCommitsEndpoint: `curl ${prefix}${
      webhook.postPullRequestCommitsEndpoint
    } -X 'POST' -H 'Authorization: Bearer ${apiKey ?? '{API_KEY}'}' -d '{
      "pullRequestKey": 1,
      "commitSha": "b22f772f1197edfafd4cc5fe679a2d299ec12837",
      "commitAuthorName": "Jane Doe",
      "commitAuthorEmail": "jane.doe@example.com",
      "commitAuthoredDate": "2025-02-20T16:17:36Z"
    }'`,
    postPullRequestCommentsEndpoint: `curl ${prefix}${
      webhook.postPullRequestCommentsEndpoint
    } -X 'POST' -H 'Authorization: Bearer ${apiKey ?? '{API_KEY}'}' -d '{
      "id": "pr-comment-1",
      "pullRequestKey": 1,
      "body": "Looks good to me.",
      "accountId": "reviewer-1",
      "createdDate": "2025-02-20T16:45:00Z",
      "type": "REVIEW",
      "status": "APPROVED"
    }'`,
  };
